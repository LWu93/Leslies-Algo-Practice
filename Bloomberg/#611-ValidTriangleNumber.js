...
the sum of 2 sides of a triangle MUST BE bigger than its 3rd side in order for it to be a validTringle.

Brute Force - 3 for loops to go through each num and see if they satisfy the 3 cases above.

Optimal Approach
Sort and use 2 pointers - essentially 3 sum but we're looking for a+b > c
nums.sort(a-b)

for (let start = 0 to len-2)
  for (mid = start+1 to len-1)
    let end = mid + 1
    while arr[start] + arr[mid] > end
      count++

First Approach - backtracking

basecase - when the array we're tracking have 3 nums. If more, we can stop/return.
    check if the first two elems < 3rd ele.
    IF TRUE, increment our count
    ELSE return

    for loop through array
    push the first ele in our trackingArr
    backtrack(numsArray, i+1)
    pop out of our numsArray
*/

var triangleNumber = function(nums) {
  let count = 0;

  if (!nums.length) return count;

  nums.sort((a,b) => a-b)

  //2 pointers approach + while loop to account for the insides. basically 3sum with a different condition
  for (let start = 0; start < nums.length - 2; start++) {
    for (let mid = start+1; mid < nums.length-1 && nums[start]; mid++) {
      let end = mid + 1;

      //since its sorted, if the next end val is not greater than start+mid, it will never be. We can exit the loop there
      while (end < nums.length && nums[start] + nums[mid] > nums[end]) {
        count++
        end++
      }
    }
...
Time - O(n^2). 2 for loops. the while loop only technically runs through n elements ONCE for a 2N runtime for the nested for loop ~~ O(N) for all runs inside the second loop.
Space - O(log n). Technically, sort takes up log n space recursively.
*/
...
//         for (let i = currIdx; i < numsArr.length; i++) {
//             let curr = numsArr[i]
//             currNums.push(curr)
//             backtrack(numsArr, i+1, currNums);
//             currNums.pop()
//         }
//     }
