...
res = [1, 10, 2, 8 , 3, ...  0 0 0 0 0 ]

using a pointer to keep track of nonzeros.
       i
arr = [1, 10, 2, 0, 8, 3, 0, 0, 6, 4, 0, 5, 7, 0]
       ^
let zeroPtr = 0


I will use zeroPtr to track of ele's that are NOT zeros
I want to move the ptr every time I hit a nonZero
If I do hit a nonzero, I want to increment my zeroPtr
else i dont incremenet

for loop to arr.length
  if currVal is NOT a zero,
    swap the ptrs
    zeroPtr++
...
function moveZerosToEnd(arr) {
	...
	let zeroPtr = arr.indexOf(0);

  if (zeroPtr == -1) return arr; //edgecase where first idx is not a 0

  for (let i = zeroPtr+1; i < arr.length; i++) {
    if (arr[i] !== 0) {
      swap(arr, i, zeroPtr);
      zeroPtr++
    }
...
//Space - O(1). Constant swap and ptr.
