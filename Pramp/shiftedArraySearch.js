...
function shiftedArrSearch(shiftArr, num) {
  let length = shiftArr.length-1;
  let pivotIdx = findPivot(shiftArr, length)

  function findPivot(arr, end) {
    let start = 0

    while (start <= end) {
      let mid = start + Math.floor((end-start)/ 2)
      //if we hit the beginning or the point where its NOT sorted aka mid-1 is greater than mid.
  ...
  function binarySearch(arr, start, end) {
    ...
    //we didn't find the num
    return -1
  }

  //binary search from the pivotpoint
  if (pivotIdx == -1 || num < shiftArr[0]) {
    return binarySearch(shiftArr, pivotIdx, length)
  } else {
    return binarySearch(shiftArr, 0, pivotIdx+1)
  }
}
